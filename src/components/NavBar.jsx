  ...
  if (cookies.jwt && !userInfo) {
    const getUserInfo = async () => {
      try {
        ...
        console.log("Dispatching SET_USER with", projectedUserInfo);
        dispatch({
...
useEffect(() => {
  // If there is no JWT cookie, mark as loaded so navbar shows correctly
  if (!cookies.jwt) {
    setIsLoaded(true);
  }
}, [cookies.jwt]);
...
console.log("isLoaded:", isLoaded);
  ...
  return (
    <>
    {isLoaded&&(
    <nav className={`w-full px-24 flex justify-between items-center py-6  top-0 z-30 transition-all duration-300 ${
            navFixed || userInfo
